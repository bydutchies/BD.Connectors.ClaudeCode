...
public class StructuredOutputSchemaTests
{
    ...
    public async Task Object_ThrowsForInvalidPropertyName()
    {
        var action = () => StructuredOutputSchema.Map(
            new Dictionary<string, StructuredOutputSchema> { [" "] = StructuredOutputSchema.PlainText() });
    ...
    {
        var action = () => StructuredOutputSchema.Map(
            new Dictionary<string, StructuredOutputSchema> { [nameof(ScoreStatusResponse.Status)] = StructuredOutputSchema.PlainText() },
    ...
    {
        const string multiLinePropertyName = "Status\nCode";
        var schema = StructuredOutputSchema.Map(
            new Dictionary<string, StructuredOutputSchema> { [multiLinePropertyName] = StructuredOutputSchema.PlainText() },
            required: [multiLinePropertyName]);

        var json = schema.ToJsonObject();

        await Assert.That(json["required"]![0]!.GetValue<string>()).IsEqualTo(multiLinePropertyName);
    }
}
